...
class ExampleWidget extends JPanel implements SpotListener {
	...
	ExampleWidget(){
		...
		def reset_message_panel = new JPanel()
		reset_message_panel.setLayout(new BorderLayout())

		def reset_button = new JButton("Restart")
	...
	private void resetGame(){
		...
		def secret_x = (int) (Math.random() * board.getSpotWidth())
		def secret_y = (int) (Math.random() * board.getSpotWidth())
	...
	void spotClicked(Spot spot){
		...
		if(spot.isEmpty())
			message.setText(player_name + " cleared the spot at " + spot.getCoordString() + ". "
					+ next_player_name + " to play.")
		else{
			if(gameWon){
				def score = board.getSpotWidth() * board.getSpotHeight()
				...
				message.setText(player_name + " found the secret spot at " + spot.getCoordString() + ". " + "Score: "
						+ score + ". Game over.")
			}else
				message.setText("Spot at " + spot.getCoordString() + " is not the spot. Better luck next time. "
						+ next_player_name + " to play.")
		}
	}
